    ...
    if(value && value.xhr) {
      ...
      Promise.watch(value, function(promises) {
        ...
        this.deliver(result);
              ...
              typeOf(value) == "object" &&
              Object.clone(value).getValues().some(Promise.isPromise)) {
      // if handed an object look for promises in the values - not recursive
      Promise.watch(Object.clone(value).getValues(), function(promises) {
        this.deliver(Object.clone(value).map(Promise.getValue).getClean());
      }.bind(this));
...
var $if = function(test, trueExpr, falseExpr) {
  if(test) {
    if(typeOf(trueExpr) == "function") return trueExpr(test);
    return trueExpr;
  } else if(falseExpr) {
    if(typeOf(falseExpr) == "function") return falseExpr(test);
    return falseExpr;
  }
  return undefined;
}.future();
