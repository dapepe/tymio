/**
 * @class gx.com.Wizard
 * @description Create a dynamic wizard with pages.
...
gx.com.Wizard = new Class({
	...
	theme: {
		style: 'dialog', // outer styling class
	...
	initialize: function (display, width, options) {
		if ( !isNumber(width) || width == 0 )
			alert('gx.com.Wizard error unsupported width: ' + width);
		...
		try {
			...
			this._display.root.addClass(this.theme.style);
			...
			gx.util.Console('gx.com.Wizard->initialize', e.message);
		}
	},
    ...
    addPage: function(name, title, element, buttons) {
		...
		var page = new Element('div', {'class': 'page'}).adopt(element);
